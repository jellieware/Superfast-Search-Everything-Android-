...
while [[ "$holddata" != "." ]]; do
  read -n 1 -s -r -p "Enter another character ('.' to Quit | ',' to Update | '1' All | '2' Images | '3' Audio | '4' Video | '5' Docs | '6' Archives ): " holddata
printf "\033c"  
   if [ "$holddata" == "1" ]; then
   extensions=""
   extensions=$ALL
  ...
  if [[ "$holddata" == "1" || "$holddata" == "2" || "$holddata" == "3" || "$holddata" == "4" || "$holddata" == "5" || "$holddata" == "6" ]]; then
    if [ "${#result}" -gt 0 ]; then
    result="${result::-1}"
    fi
    fi 
